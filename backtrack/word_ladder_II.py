#!/usr/bin/env python
# -*- coding:utf-8 -*-
# @FileName  :word_ladder_II.py.py
# @Time      :2022/11/10 15:57
...
class Solution:
    def findLadders(self, beginWord: str, endWord: str, wordList: List[str]) -> List[List[str]]:
        ...
        if endWord not in wordList:
            return []
        word_map = self.create_wordmap(set(wordList + [beginWord]))
        result = self.find_path(beginWord, endWord, word_map)
        return result

    def find_path(self, begin_word, end_word: str, word_map: dict):
        word_queue = deque([begin_word])
        size_queue, flag = len(word_queue), True
        exist_set = set()
        result = []
        parent = defaultdict(set)
        # min_depth = float('inf')
        while size_queue > 0 and flag:
            curr_set = set()
            for _ in range(size_queue):
                node = word_queue.popleft()
                for ns in word_map[node]:
                    if ns in exist_set:
                        continue
                    if ns == end_word:
                        flag = False
                    # children[node].add(ns)
                    parent[ns].add(node)
                    curr_set.add(ns)
            exist_set |= curr_set
            word_queue.extend(curr_set)
            size_queue = len(word_queue)
    ...
    def create_wordmap(self, words: set):
        word_map = defaultdict(list)
        for word in words:
            for replace in [word[:i] + "*" + word[i + 1:] for i in range(len(word))]:
                word_map[word].append(replace)
                word_map[replace].append(word)

        # 化简图
        new_wordmap = defaultdict(set)
        for word, next_steps in word_map.items():
            new_next_steps = set()
            for ns in next_steps:
                new_next_steps |= set(word_map[ns])
            new_wordmap[word] = new_next_steps
        return new_wordmap
...
if __name__ == '__main__':
    ...
    # print(s.findLadders("hit", "cog", ["hot", "dog"]))
